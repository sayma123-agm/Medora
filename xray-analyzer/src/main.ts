...
IMPORTANT: The entire report must be written in ${languageName}.`;
      ...
      let friendlyError = 'Failed to analyze the image. Please try again.';
  ...
  async speakResults(): Promise<void> {
    ...
    try {
        ...
        let friendlyError = 'Could not play the audio report.';
        if (error instanceof Error) {
   ...
   */
  private async fetchWithBackoff(url: string, options: RequestInit, retries = 3, initialDelay = 1000): Promise<Response> {
